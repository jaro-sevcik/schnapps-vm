...
function generateCode(shared : SharedFunctionInfo, graph : IR.Graph) : boolean {
    return true;
}

export function compiler(shared : SharedFunctionInfo) : boolean {
    // Build graph.
    const graph = buildGraph(shared);
    if (!graph) return false;

    // Generate code.
    if (!generateCode(shared, graph)) return false;
    return true;
}
